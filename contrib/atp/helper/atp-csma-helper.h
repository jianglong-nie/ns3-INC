 ...
 */
#ifndef ATP_CSMA_HELPER_H
#define ATP_CSMA_HELPER_H
...
/**
 * \ingroup csma
 * \brief build a set of ATPCsmaNetDevice objects
 *
 ...
 */
class ATPCsmaHelper : public PcapHelperForDevice, public AsciiTraceHelperForDevice
{
  public:
    /**
     * Construct a ATPCsmaHelper.
     */
    ATPCsmaHelper();

    ~ATPCsmaHelper() override
    {
    }
     ...
     * ATPCsmaNetDevice created through ATPCsmaHelper::Install.
     */
     ...
     *
     * Set these attributes on each ns3::ATPCsmaNetDevice created
     * by ATPCsmaHelper::Install
     */
     ...
     * \param v1 the value of the attribute to set
     *
     * Set these attributes on each ns3::ATPCsmaChannel created
     * by ATPCsmaHelper::Install
     */
    ...
    /**
     * This method creates an ns3::ATPCsmaChannel with the attributes configured by
     * ATPCsmaHelper::SetChannelAttribute, an ns3::ATPCsmaNetDevice with the attributes
     * configured by ATPCsmaHelper::SetDeviceAttribute and then adds the device
    ...
    /**
     * This method creates an ns3::ATPCsmaNetDevice with the attributes configured by
     * ATPCsmaHelper::SetDeviceAttribute and then adds the device to the node and
     ...
     * \returns A container holding the added net device.
     */
    NetDeviceContainer Install(Ptr<Node> node, Ptr<ATPCsmaChannel> channel) const;
     ...
     * \returns A container holding the added net device.
     */
    NetDeviceContainer Install(std::string nodeName, Ptr<ATPCsmaChannel> channel) const;
    ...
    /**
     * This method creates an ns3::ATPCsmaChannel with the attributes configured by
     * ATPCsmaHelper::SetChannelAttribute.  For each Ptr<node> in the provided
     * container: it creates an ns3::ATPCsmaNetDevice (with the attributes
     * configured by ATPCsmaHelper::SetDeviceAttribute); adds the device to the
     ...
     * ns3::ATPCsmaNetDevice (with the attributes configured by
     * ATPCsmaHelper::SetDeviceAttribute); adds the device to the node; and attaches
     ...
     * \returns A container holding the added net devices.
     */
    NetDeviceContainer Install(const NodeContainer& c, Ptr<ATPCsmaChannel> channel) const;
     ...
     *          ATPCsmaNetDevice should be modified to use a fixed stream
  ...
  private:
     ...
     * \returns A container holding the added net device.
     */
    Ptr<NetDevice> InstallPriv(Ptr<Node> node, Ptr<ATPCsmaChannel> channel) const;
...
template <typename... Ts>
void
ATPCsmaHelper::SetQueue(std::string type, Ts&&... args)
{

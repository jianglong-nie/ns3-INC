...
// Network topology
// n0,n1(job1), m0,m1(job2), a0,a1(job3), b0,b1(job4) and c0,c1(job5) to n2, n2 to n3
//
// - Flow from n0,n1(job1), m0,m1(job2), a0,a1(job3), b0,b1(job4) and c0,c1(job5) to n3 using BulkSendApplication.
...
NS_LOG_COMPONENT_DEFINE("ATP-P2P-5job");
...
Measurement(Ptr<ATPPacketSink> sink)
{
    ...
    uint64_t currentTimeJob5Bytes = sink->GetTotalRxJob(5);  // job5
    ...
    uint64_t ReceivedJob5BytesPer100ms = currentTimeJob5Bytes - lastTimeJob5Bytes;
    ...
    SinkBytesStream_job5 << now.GetMicroSeconds() << "\t" << currentTimeJob5Bytes << "\t" << ReceivedJob5BytesPer100ms << std::endl;
    ...
    lastTimeJob5Bytes = currentTimeJob5Bytes;
...
static void
CwndChange_c0_job5(uint32_t oldCwnd, uint32_t newCwnd)
{
    cwndStream_c0_job5 << Simulator::Now().GetMicroSeconds() << "\t" << newCwnd << std::endl;
}

static void
CwndChange_c1_job5(uint32_t oldCwnd, uint32_t newCwnd)
{
    cwndStream_c1_job5 << Simulator::Now().GetMicroSeconds() << "\t" << newCwnd << std::endl;
}
...
main(int argc, char* argv[])
{
    ...
    cwndStream_n0_job1.open("atp-result/trace-p2p-5job/n0-job1-cwnd-p2p-5job.txt", ofstream::out | ofstream::trunc);
    cwndStream_n1_job1.open("atp-result/trace-p2p-5job/n1-job1-cwnd-p2p-5job.txt", ofstream::out | ofstream::trunc);
    cwndStream_m0_job2.open("atp-result/trace-p2p-5job/m0-job2-cwnd-p2p-5job.txt", ofstream::out | ofstream::trunc);
    cwndStream_m1_job2.open("atp-result/trace-p2p-5job/m1-job2-cwnd-p2p-5job.txt", ofstream::out | ofstream::trunc);
    cwndStream_a0_job3.open("atp-result/trace-p2p-5job/a0-job3-cwnd-p2p-5job.txt", ofstream::out | ofstream::trunc);
    cwndStream_a1_job3.open("atp-result/trace-p2p-5job/a1-job3-cwnd-p2p-5job.txt", ofstream::out | ofstream::trunc);
    cwndStream_b0_job4.open("atp-result/trace-p2p-5job/b0-job4-cwnd-p2p-5job.txt", ofstream::out | ofstream::trunc);
    cwndStream_b1_job4.open("atp-result/trace-p2p-5job/b1-job4-cwnd-p2p-5job.txt", ofstream::out | ofstream::trunc);
    cwndStream_c0_job5.open("atp-result/trace-p2p-5job/c0-job5-cwnd-p2p-5job.txt", ofstream::out | ofstream::trunc);
    cwndStream_c1_job5.open("atp-result/trace-p2p-5job/c1-job5-cwnd-p2p-5job.txt", ofstream::out | ofstream::trunc);
    SinkBytesStream_job1.open("atp-result/trace-p2p-5job/job1-sinkBytes-p2p-5job.txt", ofstream::out | ofstream::trunc);
    SinkBytesStream_job2.open("atp-result/trace-p2p-5job/job2-sinkBytes-p2p-5job.txt", ofstream::out | ofstream::trunc);
    SinkBytesStream_job3.open("atp-result/trace-p2p-5job/job3-sinkBytes-p2p-5job.txt", ofstream::out | ofstream::trunc);
    SinkBytesStream_job4.open("atp-result/trace-p2p-5job/job4-sinkBytes-p2p-5job.txt", ofstream::out | ofstream::trunc);
    SinkBytesStream_job5.open("atp-result/trace-p2p-5job/job5-sinkBytes-p2p-5job.txt", ofstream::out | ofstream::trunc);
    queueSizeStream_n2.open("atp-result/trace-p2p-5job/n2-queueSize-p2p-5job.txt", ofstream::out | ofstream::trunc);
    

    uint32_t maxBytes = 0;
    Time stopTime = Seconds(1.0) + MicroSeconds(20000); // 9.6us为一个rtt时间，近似为10us，这里跑2000个rtt

    // 设置job1、job2、job3、job4和job5初始拥塞窗口
    ...
    cwndStream_c0_job5 << initialTimestamp << "\t" << job5_initCwnd << std::endl;
    cwndStream_c1_job5 << initialTimestamp << "\t" << job5_initCwnd << std::endl;
    ...
    NodeContainer nodes;
    nodes.Create(12); // 12个节点: n0,n1,m0,m1,a0,a1,b0,b1,c0,c1,n2,n3

    NodeContainer n0n2 = NodeContainer(nodes.Get(0), nodes.Get(10)); // n0-n2
    NodeContainer n1n2 = NodeContainer(nodes.Get(1), nodes.Get(10)); // n1-n2
    NodeContainer m0n2 = NodeContainer(nodes.Get(2), nodes.Get(10)); // m0-n2
    NodeContainer m1n2 = NodeContainer(nodes.Get(3), nodes.Get(10)); // m1-n2
    NodeContainer a0n2 = NodeContainer(nodes.Get(4), nodes.Get(10)); // a0-n2
    NodeContainer a1n2 = NodeContainer(nodes.Get(5), nodes.Get(10)); // a1-n2
    NodeContainer b0n2 = NodeContainer(nodes.Get(6), nodes.Get(10)); // b0-n2
    NodeContainer b1n2 = NodeContainer(nodes.Get(7), nodes.Get(10)); // b1-n2
    NodeContainer c0n2 = NodeContainer(nodes.Get(8), nodes.Get(10)); // c0-n2
    NodeContainer c1n2 = NodeContainer(nodes.Get(9), nodes.Get(10)); // c1-n2
    NodeContainer n2n3 = NodeContainer(nodes.Get(10), nodes.Get(11)); // n2-n3
    ...
    NetDeviceContainer d0d2, d1d2, m0d2, m1d2, a0d2, a1d2, b0d2, b1d2, c0d2, c1d2, d2d3;
    ...
    // Install c0,c1 to n2 links with 2us delay
    pointToPoint.SetChannelAttribute("Delay", StringValue("2us"));
    c0d2 = pointToPoint.Install(c0n2);
    c1d2 = pointToPoint.Install(c1n2);
    ...
    Ipv4InterfaceContainer ic0i2 = ipv4Helper.Assign(c0d2);

    ipv4Helper.SetBase("10.1.10.0", "255.255.255.0");
    Ipv4InterfaceContainer ic1i2 = ipv4Helper.Assign(c1d2);

    ipv4Helper.SetBase("10.1.11.0", "255.255.255.0");
    Ipv4InterfaceContainer i2i3 = ipv4Helper.Assign(d2d3);

    //
    // Create a PacketSinkApplication and install it on node 11 (n3)
    //
    ...
    Ptr<Socket> sinkSocket = Socket::CreateSocket(nodes.Get(11), ATPSocketFactory::GetTypeId());
    ...
    nodes.Get(11)->AddApplication(sinkApp);

    //
    // Create sockets for job1, job2, job3, job4 and job5
    //
    ...
    Ptr<Socket> c0job5socket = Socket::CreateSocket(nodes.Get(8), ATPSocketFactory::GetTypeId());
    Ptr<Socket> c1job5socket = Socket::CreateSocket(nodes.Get(9), ATPSocketFactory::GetTypeId());
    ...
    Ptr<ATPSocket> c0job5_ATPSocket = DynamicCast<ATPSocket>(c0job5socket);
    Ptr<ATPSocket> c1job5_ATPSocket = DynamicCast<ATPSocket>(c1job5socket);
    ...
    Ptr<ATPBulkSendApplication> c0job5App = CreateObject<ATPBulkSendApplication>();
    Ptr<ATPBulkSendApplication> c1job5App = CreateObject<ATPBulkSendApplication>();
    ...
    uint16_t sendPort5 = 15; // Port for job5
    ...
    Address c0Address(InetSocketAddress(ic0i2.GetAddress(0), sendPort5));  // 使用端口15
    Address c1Address(InetSocketAddress(ic1i2.GetAddress(0), sendPort5));  // 使用端口15
    ...
    // 设置job5初始拥塞窗口
    c0job5_ATPSocket->SetInitCwnd(job5_initCwnd);
    c1job5_ATPSocket->SetInitCwnd(job5_initCwnd);

    // Configure c0job5App
    c0job5App->Setup(sinkAddress, c0job5_ATPSocket, maxBytes, 5);  // Note jobId = 5
    c0job5App->SetEnableATPTag(true);
    c0job5App->SetStartTime(Seconds(1.0));  // Start at the same time as other jobs
    c0job5App->SetStopTime(stopTime);
    c0job5App->SetFaninDegree(0b00000011); // 2个发送端
    c0job5App->SetWorkerId(0b00000001);

    c0job5_ATPSocket->SetConnectCallback(MakeCallback(&ATPBulkSendApplication::ConnectionSucceeded, c0job5App),
                                MakeCallback(&ATPBulkSendApplication::ConnectionFailed, c0job5App));
    c0job5_ATPSocket->SetSendCallback(MakeCallback(&ATPBulkSendApplication::DataSend, c0job5App));
    c0job5_ATPSocket->Bind(c0Address);
    c0job5_ATPSocket->Connect(sinkAddress);

    nodes.Get(8)->AddApplication(c0job5App);

    // Configure c1job5App
    c1job5App->Setup(sinkAddress, c1job5_ATPSocket, maxBytes, 5);  // Note jobId = 5
    c1job5App->SetEnableATPTag(true);
    c1job5App->SetStartTime(Seconds(1.0));  // Start at the same time as other jobs
    c1job5App->SetStopTime(stopTime);
    c1job5App->SetFaninDegree(0b00000011); // 2个发送端
    c1job5App->SetWorkerId(0b00000010);

    c1job5_ATPSocket->SetConnectCallback(MakeCallback(&ATPBulkSendApplication::ConnectionSucceeded, c1job5App),
                                MakeCallback(&ATPBulkSendApplication::ConnectionFailed, c1job5App));
    c1job5_ATPSocket->SetSendCallback(MakeCallback(&ATPBulkSendApplication::DataSend, c1job5App));
    c1job5_ATPSocket->Bind(c1Address);
    c1job5_ATPSocket->Connect(sinkAddress);

    nodes.Get(9)->AddApplication(c1job5App);
    ...
    c0job5_ATPSocket->GetTxBuffer()->TraceConnectWithoutContext("cwndTrace",
         MakeCallback(&CwndChange_c0_job5));
    c1job5_ATPSocket->GetTxBuffer()->TraceConnectWithoutContext("cwndTrace",
         MakeCallback(&CwndChange_c1_job5));
    ...
    Ptr<ATPStaticRouting> staticRouting_c0 = staticRoutingHelper.GetStaticRouting(nodes.Get(8)->GetObject<Ipv4>());
    Ptr<ATPStaticRouting> staticRouting_c1 = staticRoutingHelper.GetStaticRouting(nodes.Get(9)->GetObject<Ipv4>());
    Ptr<ATPStaticRouting> staticRouting_n2 = staticRoutingHelper.GetStaticRouting(nodes.Get(10)->GetObject<Ipv4>());
    Ptr<ATPStaticRouting> staticRouting_n3 = staticRoutingHelper.GetStaticRouting(nodes.Get(11)->GetObject<Ipv4>());
    ...
    // 配置c0的路由表
    // c0到n3的路由:通过n2转发
    staticRouting_c0->AddHostRouteTo(i2i3.GetAddress(1), ic0i2.GetAddress(1), 1);

    // 配置c1的路由表
    // c1到n3的路由:通过n2转发
    staticRouting_c1->AddHostRouteTo(i2i3.GetAddress(1), ic1i2.GetAddress(1), 1);
    ...
    // n2到c0的路由
    staticRouting_n2->AddHostRouteTo(ic0i2.GetAddress(0), ic0i2.GetAddress(0), 9);
    // n2到c1的路由
    staticRouting_n2->AddHostRouteTo(ic1i2.GetAddress(0), ic1i2.GetAddress(0), 10);
    // n2到n3的路由
    staticRouting_n2->AddHostRouteTo(i2i3.GetAddress(1), i2i3.GetAddress(1), 11);
    ...
    // n3到c0的路由:通过n2转发
    staticRouting_n3->AddHostRouteTo(ic0i2.GetAddress(0), i2i3.GetAddress(0), 1);
    // n3到c1的路由:通过n2转发
    staticRouting_n3->AddHostRouteTo(ic1i2.GetAddress(0), i2i3.GetAddress(0), 1);
    ...
    sinkATPSocket->AddAddressMapping(5, ic0i2.GetAddress(0), sendPort5); // job5 - c0
    sinkATPSocket->AddAddressMapping(5, ic1i2.GetAddress(0), sendPort5); // job5 - c1

...
std::ofstream cwndStream_m1_job2;
...
main(int argc, char* argv[])
{
    ...
    cwndStream_n0_job1.open("atp-result/trace-sigmetrics/n0-job1-cwnd-p2p-2w2w-diffRTT.txt", std::ofstream::out | std::ofstream::trunc);
    cwndStream_n1_job1.open("atp-result/trace-sigmetrics/n1-job1-cwnd-p2p-2w2w-diffRTT.txt", std::ofstream::out | std::ofstream::trunc);
    cwndStream_m0_job2.open("atp-result/trace-sigmetrics/m0-job2-cwnd-p2p-2w2w-diffRTT.txt", std::ofstream::out | std::ofstream::trunc);
    cwndStream_m1_job2.open("atp-result/trace-sigmetrics/m1-job2-cwnd-p2p-2w2w-diffRTT.txt", std::ofstream::out | std::ofstream::trunc);
    SinkBytesStream_job1.open("atp-result/trace-sigmetrics/job1-sinkBytes-p2p-2w2w-diffRTT.txt", std::ofstream::out | std::ofstream::trunc);
    SinkBytesStream_job2.open("atp-result/trace-sigmetrics/job2-sinkBytes-p2p-2w2w-diffRTT.txt", std::ofstream::out | std::ofstream::trunc);
    queueSizeStream_n2.open("atp-result/trace-sigmetrics/n2-queueSize-p2p-2w2w-diffRTT.txt", std::ofstream::out | std::ofstream::trunc);
    ...
    NodeContainer nodes;
    nodes.Create(7); // 7个节点: n0,n1,m0,m1,n2,n3,t0

    NodeContainer n0n2 = NodeContainer(nodes.Get(0), nodes.Get(4)); // n0-n2
    NodeContainer n1n2 = NodeContainer(nodes.Get(1), nodes.Get(4)); // n1-n2
    NodeContainer m0n2 = NodeContainer(nodes.Get(2), nodes.Get(4)); // m0-n2
    NodeContainer m1n2 = NodeContainer(nodes.Get(3), nodes.Get(4)); // m1-n2
    NodeContainer n2n3 = NodeContainer(nodes.Get(4), nodes.Get(5)); // n2-n3
    NodeContainer t0n2 = NodeContainer(nodes.Get(6), nodes.Get(4)); // t0-n2 (background traffic)
    ...
    NetDeviceContainer d0d2, d1d2, m0d2, m1d2, d2d3, t0d2;
    ...
    // Install t0 to n2 link with 2us delay (background traffic)
    pointToPoint.SetChannelAttribute("Delay", StringValue("2us"));
    t0d2 = pointToPoint.Install(t0n2);
    ...
    */
    ...
    //
    // TCP PacketSink on n3 for background traffic
    ...
    Ptr<ATPL4Protocol> atpl4_n2 = staticRouting_n2->GetATPL4Protocol();
    atpl4_n2->SetLayerId(1);
    atpl4_n2->SetAggregatorFaninDegree(1, 0b00000011);
    atpl4_n2->SetAggregatorFaninDegree(2, 0b00000011);
    ...
    // n2到t0的路由
    staticRouting_n2->AddHostRouteTo(it0i2.GetAddress(0), it0i2.GetAddress(0), 6);
    ...
    // n3到t0的路由:通过n2转发
    staticRouting_n3->AddHostRouteTo(it0i2.GetAddress(0), i2i3.GetAddress(0), 1);

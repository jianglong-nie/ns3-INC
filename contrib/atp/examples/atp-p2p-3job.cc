...
// Network topology
// n0,n1(job1), m0,m1(job2) and a0,a1(job3) to n2, n2 to n3
//
// - Flow from n0,n1(job1), m0,m1(job2) and a0,a1(job3) to n3 using BulkSendApplication.
...
NS_LOG_COMPONENT_DEFINE("ATP-P2P-3job");
...
// PacketSink端接收到的job1、job2和job3的字节数
...
main(int argc, char* argv[])
{
    ...
    cwndStream_n0_job1.open("atp-result/trace-p2p-3job/n0-job1-cwnd-p2p-3job.txt", ofstream::out | ofstream::trunc);
    cwndStream_n1_job1.open("atp-result/trace-p2p-3job/n1-job1-cwnd-p2p-3job.txt", ofstream::out | ofstream::trunc);
    cwndStream_m0_job2.open("atp-result/trace-p2p-3job/m0-job2-cwnd-p2p-3job.txt", ofstream::out | ofstream::trunc);
    cwndStream_m1_job2.open("atp-result/trace-p2p-3job/m1-job2-cwnd-p2p-3job.txt", ofstream::out | ofstream::trunc);
    cwndStream_a0_job3.open("atp-result/trace-p2p-3job/a0-job3-cwnd-p2p-3job.txt", ofstream::out | ofstream::trunc);
    cwndStream_a1_job3.open("atp-result/trace-p2p-3job/a1-job3-cwnd-p2p-3job.txt", ofstream::out | ofstream::trunc);
    SinkBytesStream_job1.open("atp-result/trace-p2p-3job/job1-sinkBytes-p2p-3job.txt", ofstream::out | ofstream::trunc);
    SinkBytesStream_job2.open("atp-result/trace-p2p-3job/job2-sinkBytes-p2p-3job.txt", ofstream::out | ofstream::trunc);
    SinkBytesStream_job3.open("atp-result/trace-p2p-3job/job3-sinkBytes-p2p-3job.txt", ofstream::out | ofstream::trunc);
    queueSizeStream_n2.open("atp-result/trace-p2p-3job/n2-queueSize-p2p-3job.txt", ofstream::out | ofstream::trunc);
    

    uint32_t maxBytes = 0;
    Time stopTime = Seconds(1.0) + MicroSeconds(10000); // 9.6us为一个rtt时间，近似为10us，这里跑2000个rtt

    // 设置job1、job2和job3初始拥塞窗口
    ...
    NodeContainer nodes;
    nodes.Create(8); // 8个节点: n0,n1,m0,m1,a0,a1,n2,n3

    NodeContainer n0n2 = NodeContainer(nodes.Get(0), nodes.Get(6)); // n0-n2
    NodeContainer n1n2 = NodeContainer(nodes.Get(1), nodes.Get(6)); // n1-n2
    NodeContainer m0n2 = NodeContainer(nodes.Get(2), nodes.Get(6)); // m0-n2
    NodeContainer m1n2 = NodeContainer(nodes.Get(3), nodes.Get(6)); // m1-n2
    NodeContainer a0n2 = NodeContainer(nodes.Get(4), nodes.Get(6)); // a0-n2
    NodeContainer a1n2 = NodeContainer(nodes.Get(5), nodes.Get(6)); // a1-n2
    NodeContainer n2n3 = NodeContainer(nodes.Get(6), nodes.Get(7)); // n2-n3
    ...
    NetDeviceContainer d0d2, d1d2, m0d2, m1d2, a0d2, a1d2, d2d3;
    ...
    // Install m0,m1 to n2 links with 4us delay
    pointToPoint.SetChannelAttribute("Delay", StringValue("2us"));
    m0d2 = pointToPoint.Install(m0n2);
    m1d2 = pointToPoint.Install(m1n2);

    // Install a0,a1 to n2 links with 6us delay
    pointToPoint.SetChannelAttribute("Delay", StringValue("2us"));
    ...
    //
    // Create a PacketSinkApplication and install it on node 7 (n3)
    //
    ...
    //
    // Create sockets for job1, job2 and job3
    //
    ...
    Ptr<ATPStaticRouting> staticRouting_n2 = staticRoutingHelper.GetStaticRouting(nodes.Get(6)->GetObject<Ipv4>());
    Ptr<ATPStaticRouting> staticRouting_n3 = staticRoutingHelper.GetStaticRouting(nodes.Get(7)->GetObject<Ipv4>());
    ...
    // n2到n3的路由
    staticRouting_n2->AddHostRouteTo(i2i3.GetAddress(1), i2i3.GetAddress(1), 7);
